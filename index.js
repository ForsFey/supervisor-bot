...
client.on("message", async message => { 
  let xp = require("./xp.json");
    ...
    xp[message.author.id].xp =  suankixp + xpekle; 
    if(sonrakiseviye <= xp[message.author.id].xp){
      xp[message.author.id].level = suankiseviye + 1; 
      let seviyeatladi = new Discord.MessageEmbed()  
      .setTitle("Seviye Atladı!" + ` ${message.member.user.username}`)
      .setColor("RANDOM")
      .addField("Yeni seviyen:", suankiseviye + 1);
  
      var seviyekanali = "" //Seviye kanalı ID
      if(!seviyekanali) return;
   
      client.channels.cache.get(seviyekanali).send(seviyeatladi);
    }
